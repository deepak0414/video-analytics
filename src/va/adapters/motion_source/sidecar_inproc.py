...
the same sidecar pattern every other stub role uses. A missing file yields no
events (a camera with a quiet day), never an error.
"""
...
class SidecarMotionSource:
    ...
    def events(
        ...
        if self.events_file is None or not self.events_file.exists():
            return []
